...
;(function($){$.fn.diapopage = function(opts, callback) {
...
//The slideshow starts when all the images are loaded
	...
	if(elem.length!=0){
		...
		selector.wrapInner('<div class="pix_relativize_page" style="width:'+w+'px; height:'+h+'px" />');	//wrap a div for the position of absolute elements
					...
					if($('.diapoeased').length==blocks){
						...
						if (lMoveIn!=0){
							$('.diapocurrent .fromLeft, .diapocurrent .fromRight, .diapocurrent .fromTop, .diapocurrent .fromBottom, .diapocurrent .fadeIn').each(function(){
								...
								var ind = t.index();
								var hRel = t.parents('.pix_relativize_page_page').height();
								var wRel = t.parents('.pix_relativize_page_page').width();
