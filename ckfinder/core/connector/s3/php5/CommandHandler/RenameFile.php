...
//        }
        ...
        $bMoved = $bTumbMoved=true;
        $tumbFilePath=str_ireplace("images", "_thumbs/Images", $filePath);
        $tumbNewFilePath=str_ireplace("images", "_thumbs/Images", $newFilePath);
        global $config;
        $s3 = s3_con();
        ...
        $copy = $s3->copyObject(
            ...
            substr($filePath, 1),
            $config['AmazonS3']['Bucket'],
            substr($newFilePath, 1),S3::ACL_PUBLIC_READ
        );
        $tumb_copy = $s3->copyObject(
            $config['AmazonS3']['Bucket'],
            substr($tumbFilePath, 1),
            $config['AmazonS3']['Bucket'],
            substr($tumbNewFilePath, 1),S3::ACL_PUBLIC_READ
        );
        if ($copy === false) {
            $bMoved = false;
        }
        if ($tumb_copy===false) $bTumbMoved=false;
        $bMoved = $bMoved && $s3->deleteObject($config['AmazonS3']['Bucket'], substr($filePath, 1));

        $bTumbMoved = $bTumbMoved && $s3->deleteObject($config['AmazonS3']['Bucket'], substr($tumbFilePath, 1));
