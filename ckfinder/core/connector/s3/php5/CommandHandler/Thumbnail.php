...
class CKFinder_Connector_CommandHandler_Thumbnail extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        if ($_resourceTypeInfo->checkIsHiddenFile($fileName) || !CKFinder_Connector_CommandHandler_Thumbnail::CheckForFile($sourceFilePath)) {
        ...
        if (!CKFinder_Connector_CommandHandler_Thumbnail::CheckForFile($thumbFilePath)) {
        ...
        // $size = filesize("//cdn.exiteme.com".$thumbFilePath);
        // $head = array_change_key_case(get_headers("//cdn.exiteme.com".$thumbFilePath, TRUE));
        // $filesize = $head['content-length'];
        $tmpEndFileName=end(explode("/", $thumbFilePath));
        $endFileName=rawurlencode(end(explode("/", $thumbFilePath)));
        $encoded_thumbFilePath=str_ireplace($tmpEndFileName, $endFileName, $thumbFilePath);
        $theFile = file_get_contents("http://cdn.exiteme.com".$encoded_thumbFilePath);
        $size = strlen($theFile);

        $sourceImageAttr = getimagesize("http://cdn.exiteme.com".$encoded_thumbFilePath);
        ...
        // TODO: check if 304 doesn't creates problems with the thumbnail.
        // $is304 = false;
    ...
    public static function CheckForFile($destPath){
        global $SITE;
        global $bucket;

        
        $thePath = "http://cdn.exiteme.com".$destPath;
    ...
    {
        // Check if the source file starts with /
        if($sourceFile[0] == "/")
            $sourceFile = substr($sourceFile, 1);
        $tmpEndFileName=end(explode("/", $sourceFile));
        $endFileName=rawurlencode(end(explode("/", $sourceFile)));
        $encodedFileName=str_ireplace($tmpEndFileName, $endFileName, $sourceFile);
        
        $sourceImageAttr = @getimagesize("http://cdn.exiteme.com.s3.amazonaws.com/" . $encodedFileName);
        ...
        // Copy the image to local server
        $image_data = file_get_contents($baseUrl . "/" . $encodedFileName);
        $fileName = end(explode("/", $sourceFile));
        $tmp_FolderPath=str_ireplace("exitetogo/".$SITE['S3_FOLDER']."/userfiles/Images/","",$sourceFile);
        $tmp_FolderPath=str_ireplace($fileName, "",$tmp_FolderPath);   
        
        $encodedFileNameEnd=rawurlencode($fileName);

        file_put_contents($tempFolder."/".$encodedFileNameEnd,$image_data);
        // Resize the image.
        $thumbFile = $tempFolder."/".$encodedFileNameEnd;
        $thumbFileRes = $tempFolder."/"."thum_".$encodedFileNameEnd;

        $result = system("convert $thumbFile -resize " . $oSize["Width"] ."x". $oSize["Height"]. " -quality 95 -strip $thumbFileRes",$retval);


        $_config = & CKFinder_Connector_Core_Factory::getInstance("Core_Config");
        $thumbnailsConfig = $_config->getThumbnailsConfig();
        $destPath = substr($thumbnailsConfig->getDirectory() . "Images/".$tmp_FolderPath . $fileName, 1);
        //$destPath = substr($thumbnailsConfig->getDirectory() . "Images/" . $fileName, 1);
        global $config;
        $s3 = s3_con();
        $result = $s3->putObjectFile($thumbFileRes, $config['AmazonS3']['Bucket'], $destPath, S3::ACL_PUBLIC_READ);
