...
$P_DETAILS[CollageEasing]=$CONTENT_OPTIONS->CollageEasing;
$P_DETAILS[DynamicHeight]=$CONTENT_OPTIONS->DynamicHeight;
$P_DETAILS[show_pinterest_button]=$CONTENT_OPTIONS->ShowPinterestButton;
if (!$P_DETAILS[CollageEasing]) $P_DETAILS[CollageEasing]='easeInOutExpo';
$P_DETAILS[EasingSpeed]=$CONTENT_OPTIONS->EasingSpeed;
if (!$P_DETAILS[EasingSpeed]) $P_DETAILS[EasingSpeed]=800;
$selected_easing[$P_DETAILS[CollageEasing]]="selected";
...
if ($P_DETAILS[DynamicHeight]==1 OR $P_DETAILS[DynamicHeight]=="") $is_dynamic_height="checked";
...
$num_short_content_in_line=$containerWidth/($boxWidth+$P_DETAILS[ContentMarginW]+5);
...
if ($P_DETAILS[PageStyle]==1) {
	$containerWidth=$dynamicWidthPadding;
	if (intval($isLeftColumn)>0) $containerWidth=$containerWidth-$customLeftColWidth;
...
.custom {
	width:<?=($boxWidth);?>px;
	<?
	if ($P_DETAILS[DynamicHeight]==1 OR $P_DETAILS[DynamicHeight]=="") {
		?>
		height:auto;
		<?
	}
...
if (isset($_SESSION['LOGGED_ADMIN']) OR $P_DETAILS[CollageEasing]=="none") $collageAnimated="false";
...
<script>
	var collage_height;
	var collage_animated="<?=$collageAnimated;?>";
	
	function setCollageHeight() {
		collage_height=jQuery('.leftColumn_right').height();
		jQuery('.leftColumn').css('min-height',(collage_height)+"px");
	}
	function hideMe() {
		jQuery("#boxes").before('<div class="images_loading_waiting"><?=$labels[$SITE[align]][0];?></div>');
		 jQuery("#boxes").css("display","none");
	}
	function goMason() {
		jQuery('#boxes').masonry({ itemSelector: '.portlet',isAnimated:<?=$collageAnimated;?>,isRTL:<?=$isRTL;?>,columnWidth:1,animationOptions:{queue:true,duration:<?=$P_DETAILS[EasingSpeed];?>,easing:'<?=$P_DETAILS[CollageEasing];?>'}});
		
		
	}
	function goMasonNoEffect() {
		jQuery('#boxes').masonry({ itemSelector: '.portlet',isAnimated:false,isRTL:<?=$isRTL;?>,columnWidth:1});
	}
	function animateMason() {
		jQuery("#boxes").css("display","block");
		jQuery('#boxes').masonry('reload');
		jQuery(".images_loading_waiting").remove();
	}
	var mobileEnabled="<?=$SITE[mobileEnabled];?>";
	var wSize=jQuery(window).width();
	if (collage_animated=="true" && mobileEnabled=="" && wSize>680) hideMe();		
		...
		short_cont_container.imagesLoaded(function() {
			if (collage_animated=="true" && mobileEnabled=="" && wSize>680) {
				window.setTimeout('goMason()',150);
				
				window.setTimeout('animateMason()',350);
		...
		window.setTimeout('setCollageHeight()',1200);
		//jQuery(".images_loading_waiting").remove();
        });
		...
		jQuery(function() {
			...
			stop:   function(event, ui) { 
				// ui.item.removeClass('dragging').addClass('layout-card');
				 ui.item.parent().masonry('reload',{isRTL:false,isAnimated:true});
			      }
...
<?
	...
	?>
	<div id="topShortContent" style="padding-<?=$SITE[align];?>:6px;margin-<?=$SITE[opalign];?>:3px;" align="<?=$SITE[align];?>" class="mainContentText" style="margin-right:1px;"><?=$P_TITLE[Content];?></div>
	
	<?
	if (isset($_SESSION['LOGGED_ADMIN'])) {
		$ADMIN_TRANS['edit_temp']=$ADMIN_TRANS['edit'];
		$ADMIN_TRANS['move_temp']=$ADMIN_TRANS['change order'];
		$ADMIN_TRANS['del_temp']=$ADMIN_TRANS['delete content'];
		$ADMIN_TRANS['edit']="";
		$ADMIN_TRANS['change order']="";
		$ADMIN_TRANS['delete content']="";
	}
	print '<ul id="boxes">';
	...
	for ($a=0;$a<$briefs_limit;$a++) {
		...
		if ($P_DETAILS[ContentPhotoBGColor]) $topRoundedCornersColor=$P_DETAILS[ContentPhotoBGColor];
		if ($P_DETAILS[ContentTextBGColor]) $bottomRoundedCornersColor=$P_DETAILS[ContentTextBGColor];
		if ($is_rounded AND !$P_DETAILS[ContentPhotoBGColor] AND !$SITE[shortcontentbgcolor] AND !$P_DETAILS[ContentPicBG] AND $P_DETAILS[ContentTextBGColor]) $photoHolderExtraCSS=";margin-".$SITE[align].":-6px"; //Added 2/9/2012
		if ($CONTENT[isFullWidth][$a]==1) {
			$pFloating=$cFloating=$SITE[align];
			$containerWidth=$short_text_width+$boxWidth-($P_DETAILS[ContentMarginW])-12;
			if (intval($isLeftColumn)>0) $containerWidth=$containerWidth-10;
			//$padding_right=6;
		...
		if ($CONTENT[ContentPhotoName][$a]=="") {
			...
			if ($CONTENT[isFullWidth][$a]==1 AND !$is_rounded) $short_content_extra_css.=";padding-".$SITE[align].":6px;width:".($roundedCornersWidth-6)."px";
			...
			if ($P_DETAILS[isTitlesAbove]==1 AND !$CONTENT[isFullWidth][$a]==1) {
				if (isset($_SESSION['LOGGED_ADMIN']) OR $titleShow=="")
					{
					?>
					<h2 class="shortContentTitle customTitleStyle topShortContentTitle" id="titleContent_<?=$CONTENT[PageID][$a]; ?>"  style="display:<?=$titleShow;?>;padding-<?=$SITE[align];?>:<?=$short_content_padding;?>px;padding-top:3px;padding-bottom:5px">
					<?
					if (!$page_url=="") print '<a href="'.$page_url.'" target="'.$target_location.'" '.$rel_code.'>';
					?>
					<?=$CONTENT[PageTitle][$a];?></a></h2>
					<?
				}
			}
			?>
		...
		if ($CONTENT[isFullWidth][$a]==0) {
			print '<div class="clear"></div>';
			$short_content_extra_css.=";padding-bottom:5px";
		}
		?>
		<div id="short_content_container_<?=$CONTENT[PageID][$a];?>" style="padding-<?=$SITE[align];?>:0px;margin-<?=$SITE[align];?>:0px;align:<?=$SITE[align];?>;padding-top:0px;<?=$short_content_extra_css;?>">
		<?
		if (!$P_DETAILS[isTitlesAbove] OR $CONTENT[isFullWidth][$a]==1 OR $have_pic==0) {
			if (isset($_SESSION['LOGGED_ADMIN']) OR $titleShow=="")
				{
				?>
				<h2 class="shortContentTitle customTitleStyle" id="titleContent_<?=$CONTENT[PageID][$a]; ?>"  style="display:<?=$titleShow;?>;padding-<?=$SITE[align];?>:<?=$short_content_padding;?>px;padding-top:3px;">
				<?
				if (!$page_url=="") print '<a href="'.$page_url.'" target="'.$target_location.'" '.$rel_code.'>';
				?>
				<?=$CONTENT[PageTitle][$a];?></a></h2>
...
?>
<div id="bottomShortContent" style="padding-<?=$SITE[align];?>:7px;padding-<?=$SITE[opalign];?>:7px;" align="<?=$SITE[align];?>" class="mainContentText" style="margin-right:1px;"><?=$P_TITLE_BOTTOM[Content];?></div>
<?
if (isset($_SESSION['LOGGED_ADMIN'])) {
	...
	<div style="display: none" id="ContentOptions" class="CatEditor settings_slider" align="<?=$SITE[align];?>" dir="<?=$SITE[direction];?>">
		...
		<div class="CenterBoxContent" style="overflow-y:auto">
		<table border="0" cellspacing="2" style="overflow-y:auto;width:100%">
			<tr>
			<td><?=$ADMIN_TRANS['tumbnail size'];?></td>
			<td>
				<input type="text" maxlength="3" value="<?=$tumbsWidth;?>" name="contentphotowidth" id="contentphotowidth" style="width:50px;direction:ltr"/>&nbsp; X&nbsp;
				<input type="text" maxlength="3" value="<?=$tumbsHeight;?>" name="contentphotoheight" id="contentphotoheight" style="width:50px;direction:ltr"/>
				<br>
			...
			<td>
				...
				<input type="text" maxlength="3" value="<?=$P_DETAILS[ContentMarginH];?>" name="contentmarginheight" id="contentmarginheight" style="width:50px;direction:ltr"/>
				<br>
				<small><?=$ADMIN_TRANS['width in pixels'];?>
				&nbsp;
				<?=$ADMIN_TRANS['height in pixels'];?></small>
			...
			<td>
				<input type="text" maxlength="3" value="<?=$P_DETAILS[ContentMinHeight];?>" name="content_min_height" id="content_min_height" style="width:50px;direction:ltr"/>
				<br>
				<small><?=$ADMIN_TRANS['height in pixels'];?></small>
			</td>
			</tr>
			<tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['content and photo bg color'];?>: </td><td><?PickColor("P_DETAILS[ContentBGColor]",$P_DETAILS[ContentBGColor]);?></td>
			</tr>
			<tr>
			<td><?=$ADMIN_TRANS['border color'];?>: </td><td><?PickColor("P_DETAILS[ContentBorderColor]",$P_DETAILS[ContentBorderColor]);?></td>
			</tr>
			<tr>
			<td><?=$ADMIN_TRANS['photos border color'];?>: </td><td><?PickColor("P_DETAILS[PhotosBorderColor]",$P_DETAILS[PhotosBorderColor]);?></td>
			</tr>
			<tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['photos bg color'];?>: </td><td><?PickColor("P_DETAILS[ContentPhotoBGColor]",$P_DETAILS[ContentPhotoBGColor]);?></td>
			</tr>
			<tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['briefs text bg color'];?>: </td><td><?PickColor("P_DETAILS[ContentTextBGColor]",$P_DETAILS[ContentTextBGColor]);?></td>
			</tr>
			<tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['titles text color'];?>: </td><td><?PickColor("P_DETAILS[TitlesColor]",$P_DETAILS[TitlesColor]);?></td>
			</tr>
			<tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['content text color'];?>: </td><td><?PickColor("P_DETAILS[ContentTextColor]",$P_DETAILS[ContentTextColor]);?></td>
			</tr>
                        <tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['easing effect'];?>: </td>
                            <td>
                                <select name="easingEffect" id="easingEffect" value="<?=$P_DETAILS[CollageEasing];?>" style="width:75px">
                                    <?
					...
					print '<option value='.$EASINGS[$e].' '.$selected_easing[$EASINGS[$e]].'>'.$EASINGS[$e].'</option>';
                        ...
                        <tr>
			<td style="width:160px;"><?=$ADMIN_TRANS['slides speed'];?>: </td><td><input class="StyleEditFrm" name="easing_speed" id="easing_speed" value="<?=$P_DETAILS[EasingSpeed];?>" type="text" style="width:50px"></td>
			</tr>
                        <tr>
			...
			<tr>
			<tr>
			<td><?=$ADMIN_TRANS['rounded corners'];?>: </td><td><input type="checkbox" name="is_rounded_corners" id="is_rounded_corners" value=1 <?=$is_rounded_checked;?> /> </td>
			</tr>
			<tr>
			...
			<tr>
			<td><?=$ADMIN_TRANS['use original photo height'];?>: </td><td><input type="checkbox" name="is_dynamic_height" id="is_dynamic_height" value=1 <?=$is_dynamic_height;?> /> </td>
			</tr>
			...
			<tr>
				<td colspan="3"><input type="checkbox" id="show_pinterest_button" name="show_pinterest_button" <?=$showPinterestButton_check;?>>
					<img src="http://cdn.exiteme.com/images/pinterest-icon.png" border="0" height="15" align="absmiddle" /> <?=$ADMIN_TRANS['show pinterest on photo hover'];?></td>
			</tr>
			<tr>
				<td colspan="3"><input type="checkbox" id="crop_mode" name="crop_mode" <?=$cropModeChecked;?>>
					<i class="fa fa-crop"></i> <?=$ADMIN_TRANS['crop tumbs images'];?></td>
			</tr>	
			<tr>
				<td colspan="8"><br></td>
			</tr>
			<tr>
				<td colspan=2><a href="#" onclick="resetDefaultOptions()"><?=$ADMIN_TRANS['reset options from default template'];?></a></td>
			</tr>
			<tr>
			<td colspan="2"><input type="checkbox" name="is_default_options" id="is_default_options" value=1 <?=$is_default_options_checked;?> /> <?=$ADMIN_TRANS['set these options as default for new templates'];?></td>
			</tr>
			<tr>
				<td colspan="4">
			<div class="button" style="padding:5px;" onclick="jQuery('#embedSearchCode').toggle()"><?=$ADMIN_TRANS['search html code'];?></div>
				<div id="embedSearchCode" style="display:none">
					
						<textarea readonly="readonly" id="embedCode"><?=htmlspecialchars('<iframe allowtransparency="true" border="0" frameborder="0" id="search_content_attr_iframe" scrolling="no" src="'.$SITE[url].'/iframe_content_attr_search.php" width="100%"></iframe>');?></textarea>
				</div>
			<br>
		</td>
	</tr>
	...
	<div class="saveButtonsNew">
			<div class="greenSave" id="newSaveIcon"  onclick="SaveContentOptions()"><img align="absmiddle" src="<?=$SITE[url];?>/Admin/images/save-icon.png" border="0" /><?=$ADMIN_TRANS['save changes'];?></div>
			<div id="newSaveIcon" class="cancel" onclick="EditContentOptions(event)"><?=$ADMIN_TRANS['cancel'];?></div>
	</div>
	</div>
	<?
