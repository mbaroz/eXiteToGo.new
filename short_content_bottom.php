...
jQuery(window).load(function() {
	if (jQuery(window).width()>680) {
	    ...
	    jQuery('ul#boxes li').not(".wide, .li_spacer").matchHeight();
	}	
...
if (isset($_SESSION['LOGGED_ADMIN'])) {
	...
	<div style="display: none;" id="ContentOptions" class="CatEditor settings_slider" align="<?=$SITE[align];?>" dir="<?=$SITE[direction];?>">
		...
		<table border="0" cellspacing="2" style="overflow-y:auto;width:100%">
			...
			<tr>
				<td><?=$ADMIN_TRANS['full line brief width'];?>:<br><small>current: <?=$labelContainerWidth;?>px (<?=$ADMIN_TRANS['width in pixels'];?>)</small></td>
				<td><input type="text" maxlength="3" value="<?=$P_DETAILS[FullLineBriefWidth];?>" name="full_brief_width" id="full_brief_width" style="width:50px;direction:ltr"/> <small>(<?=$ADMIN_TRANS['width in pixels'];?>)</small></td>
			...
			<tr>
			 <td colspan="2"><br><input type="checkbox" name="is_default_options" id="is_default_options" value=1 <?=$is_default_options_checked;?> /> <?=$ADMIN_TRANS['set these options as default for new templates'];?>: </td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4">
					<div class="button" style="padding:5px;" onclick="ShowEmbedCode(this)"><?=$ADMIN_TRANS['embed code to insert briefs into'];?></div>
			<div id="embedGalleryCode">
				...
				<div id="d_clip_container" style="position:relative;margin-top:5px">
					...
					<div><?=$ADMIN_TRANS['source html code'];?></div>
						<textarea readonly="readonly" id="embedCode"><?=htmlspecialchars('<iframe allowtransparency="true" border="0" frameborder="0" id="iframe_shortContentBottom_'.$urlKey.'" scrolling="no" src="'.$SITE[url].'/iframe_ShortContentBottom.php?cat_url='.$urlKey.'&limit=" width="100%"></iframe>');?></textarea>
			</div>
			<br>
			<div class="button" style="padding:5px;" onclick="jQuery('#embedSearchCode').toggle()"><?=$ADMIN_TRANS['search html code'];?></div>
				<div id="embedSearchCode" style="display:none">
						<textarea readonly="readonly" id="embedSearch_Code" class="embedCode"><?=htmlspecialchars('<iframe allowtransparency="true" border="0" frameborder="0" id="search_content_attr_iframe" scrolling="no" src="'.$SITE[url].'/iframe_content_attr_search.php" width="100%"></iframe>');?></textarea>
				</div>
			<br>
		</td>
	</tr>
