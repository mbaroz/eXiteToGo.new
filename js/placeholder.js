...
Placeholder = {
    ...
    init: function(settings)
    {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if((placeholder && elements[i].type == "text" || elements[i].type == "password" || elements[i].type == "textarea") && placeholder != null && placeholder.length > 0) {
                    ...
                    if(forms[f]) {
                        ...
                        if(Placeholder.utils.contains(children, _input)) {
                            // Bind the submit to clear all empty fields
                           	jQuery(forms[f]).submit(function() {
    ...
    {
        var children = jQuery('input,textarea',form);
