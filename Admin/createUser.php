...
function AddUser($ADMIN,$uid) {
	...
	if ($uid=="") {
		$db->query("select * from users WHERE Email='$ADMIN[email]'");
		if (!$db->nextRecord()) {
			$ADMIN[securePass]=sha1($ADMIN[pass]);
			$sql="INSERT INTO users (FirstName,LastName,Email,Passwd,UserType) VALUES ('$ADMIN[fname]','$ADMIN[lname]','$ADMIN[email]','$ADMIN[securePass]',1)";
			$db->query($sql);
			print 'משתמש מערכת הוסף בהצלחה';
			?>
			<script>loadAdminPanel('listUsers');</script>
			<?
		}
		else {
			print 'שגיאה במערכת נמצא משתמש מערכת עם כתובת דואל זהה !';
		}
	}
	else {
		
		$sql="update users SET FirstName='$ADMIN[fname]',LastName='$ADMIN[lname]',Email='$ADMIN[email]' WHERE UID='$uid'";
...
if ($_GET['uid']) {
	...
	$USID=explode("|",$uID);
	$uID=$USID[1];
	$EDIT_ADMIN=GetUserDetails($uID);
	$show_pass=0;
}

if ($sent==1) AddUser($ADMIN,$_POST['uid']);
?>
...
<h2><?=$ADMIN_TRANS['edit'];?> / <?=$ADMIN_TRANS['add new user'];?></h2>
...
<input type="text" class="ConfigAdminInput HEtextbox" name="ADMIN[fname]" value="<?=$EDIT_ADMIN[FirstName];?>"></td>
</tr>
<tr>
<td class="ConfigAdmin">
<?=$ADMIN_TRANS['last name'];?>:<br>
<input type="text" class="ConfigAdminInput HEtextbox"  name="ADMIN[lname]" value="<?=$EDIT_ADMIN[LastName];?>"></td>
</tr>
<tr>
<td class="ConfigAdmin">
<?=$ADMIN_TRANS['user'];?> / <?=$ADMIN_TRANS['email'];?>:<br>
...
<tr>
<td>
	<br><input type="submit" value="<?=$ADMIN_TRANS['add new user'];?>" id="newSaveIcon" class="greenSave"></td></td>
</tr>
</table>
