...
$short_text_width=($containerWidth-$boxWidth-28);
...
#boxes li.wide {width:99.7%;}

ul#boxes li .short_content_container {
		background-color: #<?=$P_DETAILS[ContentTextBGColor];?>;
		padding:6px;
}
ul#boxes li .photoHolder {padding:0px;margin:6px;}
ul#boxes li .short_content_container .shortContentTitle {padding-<?=$SITE[align];?>:0px;}
	...
	ul#boxes li.wide .short_content_container {
		...
		margin:6px;
		box-sizing:border-box;
		width:<?=$short_text_width;?>px;
		display: block;
		max-height: 100%;
		padding:0px;

	}
	ul#boxes li.wide.nophoto .short_content_container {
		width:<?=$short_text_width+$boxWidth+18;?>px;
	}
	ul#boxes li.wide .short_content_container {
		border-radius: 0px;

	}
	ul#boxes li.wide .innerDiv {
		background-color:#<?=$P_DETAILS[ContentTextBGColor];?>;
		margin:0;
		min-height:<?=$ContentMinHeight;?>px;
		height: auto
	}
	ul#boxes li .innerDiv{background-color:#<?=$boxesBGColor;?>;}
#boxes li.li_spacer {
...
if ($is_rounded) {
	?>
	#boxes li.portlet, #boxes li.portlet .innerDiv, #boxes li.nophoto .innerDiv .short_content_container {
		border-radius: 6px;
	}
	#boxes li.portlet .innerDiv .short_content_container {
		border-bottom-right-radius: 6px;
		border-bottom-left-radius: 6px;
	}
	#boxes li.wide .innerDiv .photoHolder {border-top-<?=$SITE[align];?>-radius: 6px;border-bottom-<?=$SITE[align];?>-radius: 6px;}
	#boxes li.wide{width:94.1%}
	<?
}
?>
</style>
		...
		jQuery(function() {
			...
			stop:   function(event, ui) { 
				// ui.item.removeClass('dragging').addClass('layout-card');
				 ui.item.parent().masonry('reload',{isAnimated:true});
			      }
...
<?
	...
	for ($a=0;$a<$briefs_limit;$a++) {
		...
		$photoHolderExtraCSS="";
		$controls_margin=0;
		if ($CONTENT[ContentPhotoName][$a]=="") $class.=" nophoto";
		...
		if ($CONTENT[ContentPhotoName][$a]=="") {
			...
			if ($P_DETAILS[isTitlesAbove]==1 AND !$CONTENT[isFullWidth][$a]==1) {
				if (isset($_SESSION['LOGGED_ADMIN']) OR $titleShow=="")
					{
					?>
					<strong><div class="shortContentTitle customTitleStyle topShortContentTitle" id="titleContent_<?=$CONTENT[PageID][$a]; ?>"  style="display:<?=$titleShow;?>">
					<?
		...
		//TO BE Added 4/9
		?>
		<div class="short_content_container" id="short_content_container_<?=$CONTENT[PageID][$a];?>">
		<?
		...
		if (!$P_DETAILS[isTitlesAbove] OR $CONTENT[isFullWidth][$a]==1 OR $have_pic==0) {
			if (isset($_SESSION['LOGGED_ADMIN']) OR $titleShow=="")
				{
				?>
				<strong><div class="shortContentTitle customTitleStyle" id="titleContent_<?=$CONTENT[PageID][$a]; ?>"  style="display:<?=$titleShow;?>;">
				<?
				if (!$page_url=="") print '<a href="'.$page_url.'" target="'.$target_location.'" '.$rel_code.'>';
				?>
				<?=$CONTENT[PageTitle][$a];?></a></div></strong>
			...
			?>
			<div id="printArea">
				<div id="divContent_<?=$CONTENT[PageID][$a]; ?>" align="<?=$SITE[align];?>" class="mainContentText customContentStyle">
				<?
