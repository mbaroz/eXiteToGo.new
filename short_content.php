...
if (isset($_SESSION['LOGGED_ADMIN'])) {
	...
	function SaveContentOptions() {
		...
		var url = '<?=$SITE[url];?>/Admin/saveContentinplace.php';
		var pars = 'action=saveContentOptions&catID='+catID+'&pWidth='+pWidth+'&pHeight='+pHeight+'&content_photo_bg_color='+content_photo_bg_color+'&content_bg_color='+content_bg_color+'&titles_color='+titles_color+'&content_text_color='+content_text_color+'&hMargin='+marginH+'&isDefaultOptions='+options_default+'&full_line_width='+full_line_width+'&show_pinterest_button='+show_pinterest_button+'&images_crop_mode='+imagesCropMode;
...
<?
	...
	if (isset($_SESSION['LOGGED_ADMIN'])) {
		...
		?>
		<br />&nbsp;
		<div id="newSaveIcon" class="add_button" onclick="jQuery('.generaldropdown.popMenu').toggle()"><i class="fa fa-angle-down"></i></div><div id="newSaveIcon" class="add_button" onclick="AddNewContentType();"><i class="fa fa-file-text-o"></i> <?=$ADMIN_TRANS['add brief'];?></div>
	...
	for ($a=0;$a<count($CONTENT[PageID]);$a++) {
		...
		if (isset($_SESSION['LOGGED_ADMIN'])) {
				...
				<div class="briefs_edit" id="photo_edit_tools_<?=$a;?>" style="margin-top:<?=$controls_margin;?>px">
					...
					<div id="pDropDownMenu_<?=$a;?>" class="newSaveIcon popMenu" style="height: auto;display:none" onblur="jQuery(this).toggle();">
						...
						?>
						<?if ($SITE[enableContentAttributes]==0) {
							?>
		...
		if (isset($_SESSION['LOGGED_ADMIN']) OR $titleShow=="") 
			{
			?>
			<h2 class="shortContentTitle customTitleStyle" id="titleContent_<?=$CONTENT[PageID][$a]; ?>"  style="display:<?=$titleShow;?>;padding-<?=$SITE[align];?>:<?=$short_content_padding;?>px;">
			<?
	...
	<div style="display:none" id="ContentOptions" class="CatEditor settings_slider" align="<?=$SITE[align];?>" dir="<?=$SITE[direction];?>">
		...
		<table border="0" cellspacing="2" style="overflow-y:auto;width:100%">
			...
			<tr>
				...
				<td><input type="text" maxlength="3" value="<?=$P_DETAILS[ContentMarginH];?>" name="contentmarginheight" id="contentmarginheight" style="width:50px;direction:ltr"/> (<?=$ADMIN_TRANS['height in pixels'];?>)</td>
			</tr>
			<tr>
				<td><?=$ADMIN_TRANS['full line brief width'];?>:<br><small>current: <?=$labelContainerWidth;?>px</small></td>
				<td><input type="text" maxlength="3" value="<?=$P_DETAILS[FullLineBriefWidth];?>" name="full_brief_width" id="full_brief_width" style="width:50px;direction:ltr"/>
				(<?=$ADMIN_TRANS['width in pixels'];?>)
				</td>
			...
			<tr>
				<td colspan="3"><input type="checkbox" name="is_default_options" id="is_default_options" value=1 <?=$is_default_options_checked;?> />
				<?=$ADMIN_TRANS['set these options as default for new templates'];?><br></td>
			</tr>
