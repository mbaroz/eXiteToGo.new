...
$NumPerLine=4;
$boxHeight=$tumbsHeight+13;
$boxWidth=$tumbsWidth+11;
$textAlign="center";
$vertical=0;
$textLinesEdit=1;
$GalID=$GAL[GID];
$cursorStyle="pointer";
$photo_text_padding=2;
...
$GAL_OPTIONS[photos_border_color]=GetGalleryAttribute('photos_border_color',$GalID);
...
if ($GAL[ProductGallery]==1) {
	...
	if ($SITE[productgallerywidth]) {
		if ($SITE[productgallerywidth]<($gallery_photo_w+70)) $SITE[productgallerywidth]=$gallery_photo_w+72;
		$sideTextWidth=($galleryWidth-$SITE[productgallerywidth]-40);
		if ($P_DETAILS[PageStyle]==1) $sideTextWidth=($galleryWidth-$SITE[productgallerywidth]-30);
		$galleryWidth=$SITE[productgallerywidth];
	}
	else {
		$sideTextWidth="192";
		$galleryWidth=$dynamicWidth-490; //defaults
		if ($P_DETAILS[PageStyle]==1) {
			$sideTextWidth="200";
			$galleryWidth=$dynamicWidth-240; //defaults
		}	
	}
	$isSideTextChecked="checked";
}
...
$editor_config="config_galleries.js";
$editor_middle_config="config_galleries.js";
if (!$sideTextWidth) $sideTextWidth="500";
if ($sideTextWidth<300) $editor_config="config_side_gallery.js";
if ($galleryWidth<350) $editor_middle_config="config_side_gallery.js";
...
//if (isset($_SESSION['LOGGED_ADMIN'])) $galleryWidth=$galleryWidth+$tumbsMargin;

$numPhotosPerLine=$galleryWidth/($boxWidth+10+$tumbsMargin);
$numPhotosPerLine=round($numPhotosPerLine);
...
if (!$GAL[TumbsBGPic] AND !$GAL_OPTIONS[photo_bg_color] AND !$GAL_OPTIONS[photos_border_color] AND !$SITE[gallerybgpic] AND $GAL_OPTIONS[GalFixed]==1) {
...
<style type="text/css">
	...
	.photoName a, .photoName{
		...
		width:<?=$boxWidth+$photo_text_width_fix;?>px;
		font-weight:bold;
		overflow:hidden;
		background-color:#<?=$GAL_OPTIONS[photo_text_bg_color];?>;
		font-size: <?=$SITE[contenttextsize];?>px;
		padding-top:3px;padding-bottom:3px;
		
	}
	.photoName {
		padding-<?=$SITE[align];?>:<?=$photo_text_padding;?>px;
		padding-<?=$SITE[opalign];?>:<?=$photo_text_padding;?>px;
	}
	<?
	if (!$GAL[TumbsBGPic] AND !$SITE[gallerybgpic] AND $GAL_OPTIONS[photo_text_bg_color] AND !$GAL_OPTIONS[photo_bg_color] AND !$GAL_OPTIONS[photos_border_color]) {
		?>
		.photoName a, .photoName {
			margin-<?=$SITE[align];?>:6px;
			width:<?=$boxWidth-21;?>px;
			
		}
		.photoHolder {padding-bottom:3px;}
		<?
	}
	if ($GAL_OPTIONS[photo_text_bg_color] OR $GAL_OPTIONS[collage_gallery]) {
		?>
		.photoName a, .photoName {
			padding-top:0px;
			height:auto;
	...
	<?
	if ($GAL_OPTIONS[photo_bg_color] OR $GAL_OPTIONS[photos_border_color]) {
		?>
		.photoHolder, .photoWrapper {background-color:#<?=$GAL_OPTIONS[photo_bg_color];?>;}
		
		
		<?
		if ($GAL_OPTIONS[rounded_corners_bg]) {
			?>
			.photoHolder {border-top-left-radius:5px;border-top-right-radius:5px;}
			<?
	...
	if ($GAL[TumbsBGPic]) {
		?>
		.photoHolder {background-image:url('<?=SITE_MEDIA;?>/gallery/sitepics/<?=$GAL[TumbsBGPic];?>');}
		<?
	}
	if ($GAL[TumbsWidth]) {
		?>
		.photoHolder {width:<?=$tumbsWidth;?>px;}
		.photoWrapper {width:<?=$tumbsWidth;?>px;}
		
		<?
	}
	if ($GAL[TumbsHeight]) {
		?>
		.photoHolder {height:<?=$tumbsHeight;?>px;}
		.photoWrapper {height:<?=($tumbsHeight-0.6);?>px;}
		<?
	}
	if ($GAL_OPTIONS[photo_bg_disabled]==1) {
		?>
	...
	if ($GAL_OPTIONS[collage_gallery]) {
		?>
		.custom {height:auto;overflow-y:visible;background-repeat:repeat}
		<?
	}
	if (!$GAL_OPTIONS[collage_gallery]) {
		?>
		.photoWrapper img {max-height:<?=$tumbsHeight;?>px;}
		<?
	}
	
	?>
		.photoWrapper img {max-width:<?=$tumbsWidth;?>px;}		
	<?
	if ($GAL_OPTIONS[photos_border_color]) {
		?>
		.photoHolder {border:1px solid #<?=$GAL_OPTIONS[photos_border_color];?>;}
		.photoName {width:<?=$boxWidth+$photo_text_width_fix+2;?>px;}
		<?
		if ($GAL_OPTIONS[rounded_corners_bg]) {
			?>
			
			.photoName {border:1px solid #<?=$GAL_OPTIONS[photos_border_color];?>;border-top:0px;width:<?=$boxWidth+$photo_text_width_fix;?>px}
			<?
	...
	if (ieversion()<=8 AND ieversion()>0) {
		?>
		.photoWrapper {display:inline;line-height:<?=$tumbsHeight-2.2;?>px;}
	...
	if (($tumbsWidth!=$SITE[galleryphotowidth] OR $tumbsHeight!=$SITE[galleryphotoheight]) AND !$GAL[TumbsBGPic]) {
		?>
		.photoHolder {background-image:none;}
		<?
	}
	...
	?>
	.boxes li.ui-sortable-placeholder {background-color:transparent;border: 1px dotted silver;visibility: visible !important;min-height: 50px;width:<?=$boxWidth;?>px}
	...
	if ($SITE[mobileEnabled]) {
		...
		<?
		if (!$GAL_OPTIONS[collage_gallery] AND $GAL_OPTIONS[mobile_columns]>2) {
			?>
			.boxes li .photoName {display: none;}
			.boxes li .photoHolder {border:0px;}
			<?
		}
		else {
			?>
			.boxes li .photoName {
				...
				<?
				if (!$GAL_OPTIONS[collage_gallery]) {?>margin:0 !important;width:98% !important<?}
				?>
			}
			.boxes li .photoName a {color:white;background-color:transparent}
			.boxes li .photoHolder {border:0px;}
			<?
		}
		?>
...
<?
//if ($GAL_OPTIONS[collage_gallery] OR $show_avaliable_filters==1) {
	?>
...
//}
	...
	function hideMe() {
		jQuery(".boxes").before('<div class="spinner images_loading_waiting"><div class="bo1"></div><div class="bo2"></div><div class="bo3"></div></div>');
		 jQuery(".boxes").css("display","none");
	}
	function goMason() {
		
		//jQuery('.boxes').masonry({ itemSelector: '.portlet',isAnimated:<?=$collageAnimated;?>,isRTL:<?=$isRTL;?>,columnWidth:1,animationOptions:{queue:true,duration:<?=$GAL_OPTIONS[CollageEasingSpeed];?>,easing:'<?=$GAL_OPTIONS[CollageEasing];?>'}});
		$listContainer.isotope(
			{
			...
			itemSelector: '.portlet',
			animationEngine: 'jquery',
			animationOptions:{queue:true,duration:<?=$GAL_OPTIONS[CollageEasingSpeed];?>,easing:'<?=$GAL_OPTIONS[CollageEasing];?>'}
			}
		)
	}
...
?>
		...
		if ($GAL[GalleryName]) {
			?>
			<h1 id="galleryTitle-<?=$GAL[GID];?>"><?=$GAL[GalleryName];?></h1>
		<?
		}
		?>
		</div>
...
if ($GAL[ProductGallery]==1) {
		?>
		<div style="margin:0px 0px 0px 10px;float:<?=$SITE[opalign];?>;width:<?=$sideTextWidth;?>px;" class="gallerySideText">
		<?if (isset($_SESSION['LOGGED_ADMIN'])) {
			$enable_side_text="";
			?>
			&nbsp;<div id="newSaveIcon" onclick="EditGalleryContent('galSideText');"><img src="<?=$SITE[url];?>/Admin/images/editIcon.png" border="0" align="absmiddle" /><?=$ADMIN_TRANS['edit'];?></div>
			<span style="display:none" id="saveGalButton_side"><div  id="newSaveIcon" onclick="saveGalleryContentSide()"><img src="<?=$SITE[url];?>/Admin/images/saveIcon.gif" align="absmiddle" border="0"> <?=$ADMIN_TRANS['save'];?></div></span>
			<span style="display:none" id="closeGalButton_side"><div id="newSaveIcon" onclick="cancel()"><img src="<?=$SITE[url];?>/Admin/images/close_icon.gif" align="absmiddle" border="0"> <?=$ADMIN_TRANS['cancel'];?></div></span>
			<?
		}
		
		?>
		<div id="galSideText" style="padding-top:0px;height:100%;"><?=$GAL[GallerySideText];?></div>
		</div>
		<div id="SideGalVR">
		<?
	}
...
for ($a=0;$a<$limitPhotosPerPage;$a++){
		...
		?>
		<li <?=$gridBG;?> data-id="id-<?=$a;?>" id="photo_cell-<?=$GAL[PhotoID][$a];?>" class="portlet" <?=$last_photo_css;?> datavalue='<?=$photo_filter_name;?>' data-src='<?=$img_big_src;?>'>
		<?
		...
		$photname_bottom_rounded_css='';
		if ($GAL_OPTIONS[rounded_corners_bg]) {
			$bottom_rounded_css='border-bottom-left-radius:5px;border-bottom-right-radius:5px;';
			if ($GAL[PhotoText][$a]) $bottom_rounded_css='border-bottom-left-radius:0px;border-bottom-right-radius:0px;border-bottom:0px;';
		}
		
		?>
		<div class="photoHolder custom" align="center" style="<?=$bottom_rounded_css;?>">
		<?
		...
		<div class="photoWrapper custom" align="center" id="wrapper-<?=$a;?>">
			...
			?>
			<img id="img-<?=$a;?>" class="desaturate" <?=$src_loading;?> <?=$attr_photo;?>="<?=$img_src;?>"  border="0" align="absmiddle" style="cursor:<?=$picCursorStyle;?>" alt="<?=strip_tags($photo_alt_text);?>" />
			<?
		...
		if (isset($_SESSION['LOGGED_ADMIN'])) {
			...
			?>
			<div style="position:absolute;width:<?=$edit_controlsSpace;?>px;margin-top:-17px" align="center" class="photo_gal_editing_tools">
			<span style="float:right" class="EditPhotoIcon">
			<img onclick="EditPhotoDetails('<?=$PhotoExternalUrl;?>','<?=htmlspecialchars(html_entity_decode($GAL[PhotoText][$a]));?>',<?=$GAL[PhotoID][$a];?>,'<?=$photoProdUrlKey;?>',<?=$isProdPageChecked;?>,'<?=htmlspecialchars(html_entity_decode($GAL[PhotoAlt][$a]));?>')"  src="<?=$SITE[url];?>/Admin/images/editIcon_new.png" border="0"  class="button" title="<?=$ADMIN_TRANS['edit photo'];?>">
			</span>
			<span style="float:left">
			<img onclick="DelPhoto(<?=$GAL[PhotoID][$a];?>)" src="<?=$SITE[url];?>/Admin/images/delIcon_new.png" border="0" align="absmiddle" class="button del_button" title="<?=$ADMIN_TRANS['delete'];?>">
			</span>
			</div>
...
if ($GAL[ProductGallery]==1) {
	...
	<?if (isset($_SESSION['LOGGED_ADMIN'])) {
		...
		<?
		if ($galleryWidth<268) print '<div style="clear:both;height:10px"></div>&nbsp;&nbsp;';?>
		<?
	}
	?>
	<div id="galMiddleText" style="width:<?=($galleryWidth-33);?>px;padding-<?=$SITE[align];?>:15px;" align="<?=$SITE[align];?>" class="mainContentText galleryText"><?=$GAL[GalleryBottomPicsText];?></div>
	<?
	print '</div>';
}
?>
...
<?
if ($dsdds==1 AND !isset($_SESSION['LOGGED_ADMIN']) AND $GAL_OPTIONS[infinite_scroll]==1 AND $SITE[mobileEnabled] AND !$GAL_OPTIONS[collage_gallery] AND $mobileDetect->isMobile()) {
	?>
		<a id="next" href="/gallery_nextpage.php?p=2&urlKey=<?=$urlKey;?>&numPhotosPerPage=<?=$limitPhotosPerPage;?>"></a>
		...
		<script>
			jQuery('ul.boxes').infinitescroll({
				navSelector  	: "#next:last",
				nextSelector 	: "a#next:last",
				itemSelector 	: "ul.boxes li",
				debug		 	: false,
				dataType	 	: 'html',
				maxPage         : <?=round(count($GAL[PhotoID])/$limitPhotosPerPage);?>,
				loading: {
					finished: undefined,
					finishedMsg: "<em>Congratulations, you've reached the end of the internet.</em>",
					msg: null,
					msgText: "<div class='images_loading_waiting loading_infinite spinner'><<img src="<?=$SITE[url];?>/images/spinner.gif" /></div>",
					selector: null,
					speed: 'fast',
					start: undefined
				    },
				...
				extraScrollPx:100,
				bufferPx: 20,
				loadingText: '<div style="position:absolute;bottom:0px;left:25px;">Loading....</div>',
				pixelsFromNavToBottom:50
		
		    },function( newElements ) {
					$listContainer.isotope( 'appended', jQuery( newElements ) );
					//setGridView();
					refreshGrediView();
					 if (jQuery("a.photo_gallery").length>0) jQuery(".boxes li  a.enlarge").photoSwipe({ enableMouseWheel: true , enableKeyboard: true });
				}
		    );
		</script>
	<?
